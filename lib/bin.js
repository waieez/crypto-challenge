...
const octect = 8;
function map(binary, radix = octect, cb = _.identity, view = Uint8Array, BITS_PER_ELEMENT = 0) {
  assert.instance(binary, ArrayBuffer);
  assert.type(radix, 'number');
  assert.type([cb, view], 'function');
  const { BYTES_PER_ELEMENT } = view;
  assert.ok(BYTES_PER_ELEMENT, 'view argument should be a TypedArray');
  BITS_PER_ELEMENT = BITS_PER_ELEMENT || BYTES_PER_ELEMENT * octect;
  ...
  new view(binary).map((bits, idx) => {
    ...
    while (bitSize + remainingBits >= radix) {
      ...
      mapped.push(cb(chunk, mapped.length));
  ...
  if (remainingBits) {
    ...
    mapped.push(cb(remainder << (radix - remainingBits), mapped.length));
  }
